...
public class NotaXMLRepo extends AbstractXMLRepository<Long, Nota> {
    ...
    public Element createElementfromEntity(Document document, Nota entity) {
        ...
        e.setAttribute("idNota", entity.getId().toString());

        Element idStudent = document.createElement("idStudent");
        idStudent.setTextContent(entity.getIdStudent().toString());
        e.appendChild(idStudent);

        Element idTema = document.createElement("idTema");
        idTema.setTextContent(entity.getIdTema().toString());
    ...
    public Nota extractEntity(Element element) {
        ...
        return new Nota(Long.parseLong(id),Long.parseLong(studentId),Long.parseLong(temaId),Double.parseDouble(notaProf),dat);
        }
}
